...
# =========================================================
# IMPORTANT:
# Secrets are read ONLY from environment variables. There are NO
# hardcoded fallback secrets in this file on purpose — never commit
# real tokens, PINs or card numbers into source code.
#
# Linux / Render / Railway:
#   export BOT_TOKEN="YOUR_NEW_TOKEN"
#   export ADMIN_ID="123456789"
#   export ADMIN_PIN="YOUR_PIN"
#   export PAYMENT_CARD="YOUR_CARD_NUMBER"
#   export PAYMENT_OWNER="OWNER_NAME"
#
# Windows PowerShell:
...
#
# Render/Railway/VPS -> Environment -> shu o'zgaruvchilarni qo'shing.
#
# ESLATMA: agar avval BOT_TOKEN yoki ADMIN_PIN biror joyda (masalan,
...
if ADMIN_ID <= 0:
    raise RuntimeError(
        "ADMIN_ID noto'g'ri yoki bo'sh. Environment Variables ichida "
        "ADMIN_ID ni (o'zingizning Telegram ID raqamingizni) yozing."
    )

if not ADMIN_PIN:
    raise RuntimeError(
        "ADMIN_PIN sozlanmagan. Environment Variables ichida ADMIN_PIN "
        "(admin panelga kirish uchun maxfiy kod) yarating."
    )
...
def create_or_update_user(message: Message):
    ...
    if existing:
        cur.execute("""
            UPDATE users SET first_name=?, username=?, last_seen=? WHERE id=?
...
def referral_counts(referrer_id):
    ...
    cur.execute("""
        SELECT COUNT(*) c FROM referrals WHERE referrer_id=? AND status='completed'
    """, (referrer_id,))
...
def complete_referral(referred_id, payment_id):
    ...
    cur.execute("""
        UPDATE referrals SET status='completed', payment_id=?, rewarded_at=?
...
def activate_premium(user_id, days):
    ...
    cur = conn.cursor()
    cur.execute("""
        UPDATE users SET premium_until=?, last_seen=? WHERE id=?
...
def increment_movie_stat(user_id):
    ...
    cur = conn.cursor()
    cur.execute("""
        UPDATE users SET total_movies=total_movies+1, last_seen=? WHERE id=?
    """, (now_text(), user_id))
    conn.commit()
    conn.close()
...
# =========================================================
# REQUIRED SUBSCRIPTION CHECK (real tekshiruv)
# =========================================================
...
async def is_user_subscribed(channel, user_id):
    """Bitta kanal uchun userning haqiqatan a'zo ekanini tekshiradi."""
    try:
...
async def get_missing_required_channels(user_id):
    """Barcha majburiy kanallarni tekshirib, userning obuna bo'lmagan
    kanallari ro'yxatini qaytaradi (haqiqiy tekshiruv, aiogram get_chat_member
    orqali)."""
    ...
    for channel in channels:
        subscribed = await is_user_subscribed(channel, user_id)
        if not subscribed:
...
async def require_subscription(message: Message):
    ...
    await message.answer(text=text, reply_markup=subscription_keyboard(missing))
    return False
...
def create_payment(user_id, username, first_name, plan_key, file_id, receipt_type):
    ...
    cur.execute("""
        INSERT INTO payments(
            ...
            original_price, discount_amount, final_price, referral_discount,
            referrer_id, receipt_file_id, receipt_type, status, created_at
        )
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?, 'pending', ?)
    """, (
        user_id, username, first_name, plan_key, plan["name"], final_price,
        plan["price"], discount, final_price, 1 if referrer_id else 0,
        referrer_id, file_id, receipt_type, now_text(),
    ))
...
def approve_payment(payment_id):
    ...
    cur.execute("""
        UPDATE payments SET status='approved', approved_at=?
        WHERE id=? AND status='pending'
    """, (now_text(), payment_id))
...
def payment_stats():
    ...
    cur.execute("""
        SELECT COALESCE(SUM(discount_amount),0) s FROM payments WHERE status='approved'
    """)
...
async def notify_admin_about_receipt(
    payment_id, user_id, first_name, username, payment, file_id, receipt_type, message_text="",
):
...
async def approve_callback(callback: CallbackQuery):
    ...
    if referral:
        ...
        try:
            ...
            logging.exception("Referral reward notification failed | referrer=%s", referrer_id)
            ...
            if state.get("authenticated"):
                ...
                if step == "channel_input":
                    ...
                    try:
                        ...
                        url = (
                            invite_link
                            or (f"https://t.me/{username.lstrip('@')}" if username else "-")
                        )
...
async def admin_movies(callback: CallbackQuery):
    ...
    cur.execute("""
        SELECT code,name,category,created_at FROM movies
...
async def admin_payments(callback: CallbackQuery):
    ...
    cur.execute("""
        SELECT id, user_id, username, first_name, plan_name, price,
...
async def admin_stats(callback: CallbackQuery):
    ...
    cur.execute("SELECT COUNT(*) c FROM users WHERE last_seen >= ?",
                ((now() - timedelta(days=1)).isoformat(),))
    active_24h = cur.fetchone()["c"]

    cur.execute("SELECT COUNT(*) c FROM users WHERE created_at >= ?",
                ((now() - timedelta(days=1)).isoformat(),))
...
async def admin_add_channel(callback: CallbackQuery):
    ...
    await callback.message.answer(
        "➕ <b>KANAL QO'SHISH</b>\n\n"
        "Kanal usernameini yoki Chat ID sini yuboring.\n\n"
        "🔹 Public kanal: <code>@kanal_username</code>\n"
        "🔹 Private kanal: <code>-1001234567890|https://t.me/+INVITE</code>\n\n"
        "❗️ Bot albatta shu kanalda <b>administrator</b> bo'lishi kerak, "
        "aks holda obuna tekshiruvi ishlamaydi."
    )
    await callback.answer()
...
async def check_required_channels_startup():
    ...
    logging.info("Required subscription: %s channel(s).", len(channels))
    ...
    for channel in channels:
        try:
            ...
            logging.info(
                ...
                chat.id, getattr(chat, "title", None), bot_status,
            )
            ...
            logging.exception("Required channel access failed | id=%s", channel["chat_id"])
...
async def global_error_handler(event):
    logging.error("Unhandled aiogram error: %s", event.exception, exc_info=True)
    return True
